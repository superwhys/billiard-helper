...
func ScoresGroupRouter(scoresSvc ports.ScoresService) ginutils.Option {
	return ginutils.WithGroupHandlers(
		...
		ginutils.WithHandler(http.MethodPost, "/undo", ScoresUndoHandler(scoresSvc)),
...
// @Produce json
// @Security BearerAuth
// @Param request body request.UndoScoreRequest true "撤回分数请求体"
// @Success 200 {object} ginutils.Ret[any]
// @Router /scores/undo [post]
func ScoresUndoHandler(scoresSvc ports.ScoresService) gin.HandlerFunc {
	return ginutils.RequestHandler(func(c *gin.Context, req *request.UndoScoreRequest) {
		err := scoresSvc.UndoScore(c.Request.Context(), req)
		if handleRouterError(c, err, "scores undo handler error", errcode.ErrCodeUndoScoreFailed) {
			return
		}
		c.JSON(http.StatusOK, response.ResponseSuccess())
	})
